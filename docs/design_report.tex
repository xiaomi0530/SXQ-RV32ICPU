...
\paragraph{存储访问与 MMIO 范围}
    本工程采用独立的指令存储器、数据存储器与 MMIO 外设地址空间。IMEM 与 DMEM 均使用片上 BRAM 实现，当前容量均为 \texttt{32KiB}。地址划分如表~\ref{tab:mem_map} 所示，MMIO 寄存器映射如表~\ref{tab:mmio_map} 所示。同时，工程具备一条基于 Wishbone 协议的总线，用于连接各类设备。
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{lllX}
            ...
            & 指令存储器，\texttt{8192} 个 32 位字（32KiB），由 \texttt{imem.mem} 初始化 \\

            DMEM
            & \texttt{0x00010000}
            & \texttt{0x00010000--0x00017FFF}
            & 数据存储器，\texttt{8192} 个 32 位字（32KiB），支持字节、半字、字访问 \\
...
\paragraph{动态压缩前端机制的引入背景}
    对于标准五级流水线架构，当指令运行出现控制冲突时，往往用冲刷流水线（Pipeline Flush）的方式来解决冲突。然而，每次Flush都必然将2条跳转分支指令的后续指令无效化，须于两个周期之后方有指令重新进入EX阶段，最终会致使两个周期的浪费。\par\indent
    ...
    \end{figure}
    常用跳转分支预测有静态预测All Taken、BTFNT，动态预测Bimodal BP、PAp、PAg、GAp、GAg、GSELECT、GSHARE、TAGE等多种实现方法，然而，其绝大部分高性能实现均需要构建维护较大数据表，如BTB、BHT、PHT、GHT等，使得资源消耗与功率消耗翻倍。而之所以其涉及资源较多，又往往是因为需要记录完整跳转地址，或是为缓解仅用PC或PC与历史模式哈希得到索引导致的别名问题。因而，若可以用低资源消耗的其他方式轻易得知跳转分支目标地址，并且引入更多信息加入索引哈希，便可以在低资源消耗的情况下，获得更好的跳转分支预测准确率。\par\indent
    最好的方法其实便是在跳转分支预测的周期（即指令的IF阶段）就取得指令本身，并将指令本身加入预测，因为指令本身便隐含了指令类型、跳转分支偏置、比较寄存器等多种信息。但是，由于本工程使用Dual Port BRAM实现IMEM，故读IMEM至少会有1拍的固定延迟。对于本项目早期五级流水线版本来说，只能在IF的当拍将访问地址输入IMEM，并在下一拍拿到指令（为省下周期，本工程IMEM访问时不停止流水线，直接将这一拍当成等同于IF/ID寄存器的一拍来使用，将第IF+1周期得到的指令直传ID），从而错过IF阶段的跳转分支预测。（图~\ref{fig:5stagepipelineinstructionfetch}）
\begin{figure}[H]
    \centering
...
\end{figure}
为解决此问题，一个方案是让取指过程可以在1个周期内完成，典型实现便是使用LUTRAM构建一个ICache，即指令缓存。然而，ICache逻辑复杂，且需要将指令总线支持位宽扩充，将会引入更多资源消耗。并且LUTRAM读取也具有较长的时序路径，极有可能在100MHz下导致时序违例。\par\indent
另一个方案则是加深一级流水线，在IF级之前加入一级PreIF，将访问IMEM移至PreIF阶段，IF则负责接收IMEM吐出的指令。采取这种方法可以基本保持资源使用不变，并大大缩短所需时序路径。但加深流水，必然意味着跳转分支预测的代价变大了，将会从原先五级流水线的Flush 2拍变成Flush 3拍（图~\ref{fig:6stagepipelineandflush}），在减少周期损失上不一定合算。因此，本项目提出并采用了\textbf{\textit{动态前端压缩机制}}，使得在确保跳转分支预测可以具备指令本身的信息的同时，保持资源消耗基本不变，并使Flush代价与标准五级流水线无异。
\begin{figure}[H]
    \centering
...
\end{figure}
\paragraph{动态压缩前端机制}
为解决上述问题，本项目提出并采用动态前端压缩机制，使流水线深度可以在五、六级间视条件动态调节。一般情形时，流水线为六级流水线，完成PreIF - IF - ID - EX - MEM - WB，其中IF阶段进行跳转分支预测。而当程序从头开始运行，或发生跳转（包含标准情形EX阶段引发的跳转、跳转分支预测提前引发的跳转、预测错误导致EX阶段发生的重定向跳转）时，调节相关控制信号，使得流水线前端由PreIF - IF - ID压缩为PreIF - ID，退变为五级流水线，即PreIF访问IMEM的下一拍，IMEM吐出的指令直接跳过IF转发给ID阶段。\par\indent
同时，为了减少因为缺少IF阶段而不能结合指令本身进行分支预测的指令数，当发生前端压缩的时候，需要利用BRAM的双口特性，同时使用IMEM的32位宽取指端口和32位宽读数据端口，一次性读出两条指令，即本条触发前端压缩的指令以及本条指令的下一条正常非压缩指令。（图~\ref{fig:dynamicstagesnumberpipeline}）
\begin{figure}[H]
    \centering
...
\paragraph{低资源跳转分支预测机制的引入背景}
    在前文所述的动态压缩前端机制基础上，本设计能够在 IF 阶段取得指令本身，并在预测发生前获得指令类型、立即数字段与寄存器编号等信息。因此，跳转分支预测不必仅依赖 PC 或历史状态表，也可以利用指令编码中携带的结构化信息进行分类处理。\par\indent
...
\paragraph{低资源跳转分支预测机制}
    本设计的预测机制由指令分类单元、目标地址计算单元、Branch Predictor、Jump Predictor 以及 Return Address Stack 共同构成。PreIF 阶段发起对 IMEM 的访问，下一拍 IF 阶段取回对应指令；随后，前端进行轻量级预解码，首先基于 \texttt{instr[6:2]} 判断指令是否属于 Branch、\texttt{JAL} 或 \texttt{JALR}，再根据 \texttt{rd} 与 \texttt{rs1} 编号识别 Call 与 Ret。\par\indent
    Call 由跳转指令写回寄存器是否为 \texttt{x1} 或 \texttt{x5} 判断；Ret 的判定条件为 \texttt{JALR}、\texttt{rd=x0}、\texttt{rs1=x1 / x5} 且立即数为 0。\par\indent
...
\subparagraph{\texttt{JALR}跳转预测}
    ...
    \]
    在动态压缩前端机制下，前端可能同时观察 slot0 与 slot1 两个候选指令。为避免 slot1 的返回预测受 slot0 Call / Ret 影响，RAS 生成 shadow 形式的 slot1 栈顶结果，使 slot1 基于“slot0 生效后的前瞻栈状态”进行 Ret 目标预测。若 slot0 是 Call，则 slot1 看到的栈顶等价于压入 \texttt{slot0 PC+4} 后的结果；若 slot0 是 Ret，则 slot1 看到的栈顶等价于弹出一层后的结果。该 shadow 结果只用于前端预测，不提前修改真实 RAS 状态。\par\indent
...
\paragraph{小结}
    总体而言，本设计没有使用单一复杂预测器覆盖所有控制流指令，而是依据目标地址来源进行分类处理：可直接计算的目标不进入目标缓存，条件分支仅使用 BHT / BTFNT 预测方向，函数返回使用 RAS，普通 \texttt{JALR} 跳转使用小容量 JTB，其他 \texttt{JAL} 直算跳转。该机制与动态压缩前端配合，在较低资源开销下减少控制流恢复代价，并降低大表结构对时序路径和片上资源的压力。
    \begin{figure}[H]
        \centering
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{lcc}
            ...
            \texttt{Branch} & \texttt{87.06\%} & \texttt{100.00\%} \\
            \midrule
...
\subsection{乘法器、除法器与长延迟单元}
    本设计支持 \texttt{RV32M} 乘除法扩展，其中乘法与除法均作为执行级长延迟单元接入。ID 阶段识别到乘法或除法类指令后，在不存在冲刷、短阻塞和长保持的条件下产生 \texttt{id\_mul\_preload} 或 \texttt{id\_div\_preload}，将操作数提前送入对应执行单元。\par\indent
    这样可以使长延迟单元尽早开始工作，减少由译码到执行之间的额外等待。
...
\begin{itemize}
    \item 模块名与文件名保持一致，采用小写下划线形式，例如 \path|branch_predictor.v|、\path|mem_ctrl_resolve.v|。
    ...
    \item 全局参数和宏定义集中放在 \texttt{defines.v} 中，采用大写命名，如 \texttt{IMEM\_\allowbreak{}BASE\_\allowbreak{}ADDR}、\texttt{BR\_\allowbreak{}PRED\_\allowbreak{}INDEX\_\allowbreak{}BITS}。
\end{itemize}
...
\begin{itemize}
    ...
    \item 组合路径应避免同时包含大规模比较、加法、查表和多级选择；必要时采用流水级拆分。
...
\begin{itemize}
    ...
    \item 流水线控制类逻辑独立封装，例如 \path|ctrl_late_detect.v| 负责晚解析检测，\path|mem_ctrl_resolve.v| 负责 MEM 级控制流恢复。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lX}
        ...
        FPGA 资源占用 & \path|top_utilization_placed.rpt| \\
        FPGA 时序结果 & \path|top_timing_summary_postroute_physopted.rpt| \\
        FPGA 功耗结果 & \path|top_power_routed.rpt| \\
        布线状态与 DRC & \path|top_route_status.rpt|、\path|top_drc_routed.rpt| \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lX}
        ...
        testbench & \path|sim_1/new/cpu_tb.v| \\
        指令测试 & \path|sources_1/new/RISCV-TEST| \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lcccX}
        ...
        M 扩展 & 8 & 8 & \texttt{100\%} & \texttt{MUL} / \texttt{MULH} / \texttt{MULHSU} / \texttt{MULHU} / \texttt{DIV} / \texttt{DIVU} / \texttt{REM} / \texttt{REMU} \\
        \midrule
...
其中，CoreMark Score 使用 \texttt{5000} 次上板迭代，运行时间为 \texttt{18.121068 s}，达到 \texttt{275.921925 Iter/s}，折合 \texttt{2.759219 CoreMark / MHz}。表中的 CoreMark IPC、周期数和退役指令数则来自 \texttt{1} 次迭代仿真，不用于反推 \texttt{5000} 次上板运行时间。\par\indent
Dhrystone 达到 \texttt{166000 Dhrystones/s}，按 \(\mathrm{DMIPS}/\mathrm{MHz}=\mathrm{Dhrystones/s}/(1757\times f_{\mathrm{MHz}})\) 计算为 \texttt{0.945 DMIPS / MHz}。Embench-WikiSort 输出 \texttt{WS V=1 S=122.58}，说明排序 benchmark 结果校验通过。

\subsection{跳转分支预测统计}
分支与跳转预测统计来自 \texttt{ITERATIONS=1} 的 CoreMark Vivado 仿真窗口，而不是 \texttt{ITERATIONS=5000} 的上板 UART 性能输出。采用该口径是为了在合理仿真时间内获得周期级控制流事件统计。\par\indent
方向准确率表示控制流方向预测正确比例；目标准确率表示预测 taken 或跳转目标时目标地址正确比例；总体准确率为综合方向与目标后的控制流预测正确比例。统计结果见表~\ref{tab:validation_branch_stats}。
...
程序运行前需要将目标测试程序或 benchmark 编译为 \texttt{imem.mem}，并放置在 \path|sources_1/new| 目录下。综合和仿真时，IMEM 通过该文件初始化指令内容；DMEM 则用于运行时数据读写。当前 MMIO 地址空间用于输出运行状态、UART 结果和 benchmark 结束标志，便于在仿真和上板环境中复用同一套软件输出路径。
...
\begin{enumerate}
    ...
    \item 检查 RTL 源文件、时钟 IP、约束文件和顶层模块连接关系；
    \item 确认目标器件为 \texttt{xc7a100tcsg324-1}；
...
当前版本已经完成 \texttt{RV32IM} 单发射准六级流水线 CPU 的基本设计目标，并在 FPGA 上实现了较好的性能、面积和功耗平衡。后续优化不会简单转向复杂多发射或完整乱序结构，而将继续围绕轻量化、可综合性和真实工作负载效率展开，在保持资源开销可控的前提下进一步提高处理器可用性与性能上限。

\subsection{超微局部乱序调节机制}
基于现有动态压缩前端机制，本设计已经形成了面向六级流水线架构的超微局部乱序调节机制设想。该机制并非引入完整乱序执行窗口，而是在时序路径仍具有一定余量的前提下，对少量确定安全的相邻指令进行局部调度或执行位置调整，使部分不依赖 \texttt{load} 结果的后续指令能够绕过 \texttt{load-use hazard} 造成的固定等待。其目标是在不引入复杂重命名、提交队列和大规模相关检查结构的情况下，吸收一部分短距离访存相关带来的 stall 周期。

该方向的关键约束在于正确性和时序可控性。后续设计需要严格限定可调节指令范围，保证访存顺序、副作用顺序、控制流恢复和异常语义不会被破坏；同时，需要评估局部调节逻辑对 ID、EX 和旁路选择路径的影响。若实现得当，该机制有望成为动态压缩前端之后的进一步性能增益点，使轻量单发射流水线在不显著增加面积的条件下缓解典型 \texttt{load-use} 阻塞。

\subsection{总线机制完善与标准化}
当前版本已经具备连接 CPU、IMEM、DMEM 与 MMIO 外设的基本总线机制，能够支撑裸机 benchmark 和板级外设访问。但从工程复用和系统扩展角度看，总线接口仍需要进一步标准化，包括握手时序、错误响应、设备地址译码、读写通道约束、外设扩展方式以及跨模块命名规范等方面。

后续计划将现有总线互连进一步整理为更规范的片上互连接口，使其能够更清晰地支持多外设扩展、不同延迟设备接入和统一的仿真检查。该工作并不直接追求短期 benchmark 分数提升，但会显著改善工程可维护性，为后续接入定时器、中断控制器、UART 接收、SPI/GPIO 等外设提供稳定基础。

\subsection{系统级指令与 RTOS 支持}
当前处理器主要面向裸机运行环境，尚未实现完整特权级、CSR、异常/中断和系统同步类指令。后续将视资源、时序和应用需求，逐步引入必要的系统级机制，使处理器能够运行一个小型 RTOS。优先考虑的方向包括基础 CSR 访问、机器模式异常入口、定时器中断、外部中断、\texttt{ECALL/EBREAK/MRET} 等关键控制路径，以及与软件启动代码相匹配的中断向量和上下文保存机制。

这一阶段的目标不是一次性实现完整 Linux 级系统支持，而是形成可运行轻量实时任务调度的最小系统能力。若能够在现有低资源流水线基础上稳定运行小型 RTOS，则本设计将从 benchmark 型裸机 CPU 进一步扩展为可承担实际嵌入式控制任务的软核处理器。

\subsection{跳转分支预测准确率提升}
当前版本已经通过动态压缩前端、分类预测、Tagged BHT、JTB 与 RAS 等机制，在较低资源开销下取得了较好的控制流预测效果。后续仍可围绕分支方向预测准确率和间接跳转目标命中率继续优化，尤其是对小容量预测表中的别名冲突、冷启动行为和不同程序负载下的表项替换策略进行进一步研究。

可能的优化方向包括：改进 BHT 索引与 Tag 生成方式，评估更稳定的立即数、PC 与局部历史混合策略；针对热点分支引入更细粒度的表项更新策略，减少一次性分支对小表结构的污染；适度增强 JTB 与 RAS 的更新条件和恢复机制，提高函数指针、间接跳转和深层调用场景下的目标预测稳定性。该方向仍将坚持低资源约束，优先追求单位 LUT 性能收益，而不是直接采用大容量 BTB 或复杂全局历史预测器。

\subsection{阶段性目标}
综合来看，后续更新将围绕四个层次推进：在流水线内部，通过超微局部乱序调节机制减少部分数据相关等待；在片上互连层面，通过总线标准化提升扩展能力；在系统软件层面，通过系统级指令与中断机制支持小型 RTOS；在前端控制流层面，继续提高分支预测准确率和间接跳转预测稳定性。上述优化共同服务于同一目标：在保持小面积、低功耗和 FPGA 友好实现方式的前提下，进一步提升该 \texttt{RV32IM} 处理器在真实嵌入式场景中的性能和可用性。
