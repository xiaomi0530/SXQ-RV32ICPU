  ...
  /* Main and Proc_0 in the Ada version             */
{
  ...
  {
    unsigned long hz_per_k = (unsigned long)HZ / 1000UL;
    unsigned long cycles_per_us = (unsigned long)HZ / Mic_secs_Per_Second;
    unsigned long run_count = (unsigned long)Number_Of_Runs;
    unsigned long elapsed_cycles = (unsigned long)User_Time;

    if ((elapsed_cycles == 0UL) || (run_count == 0UL)
            || (cycles_per_us == 0UL) || (hz_per_k == 0UL)) {
      Microseconds = 0;
      Dhrystones_Per_Second = 0;
    } else {
      Microseconds = (long)((elapsed_cycles + ((cycles_per_us * run_count) / 2UL))
                   / (cycles_per_us * run_count));
      Dhrystones_Per_Second = (long)((((hz_per_k * run_count) + (elapsed_cycles / 2UL))
                            / elapsed_cycles) * 1000UL);
    }
  }
