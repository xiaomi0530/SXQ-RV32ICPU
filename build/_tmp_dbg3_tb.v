...
module cpu_tb;
    ...
    always @(posedge clk) begin
        if (cycle_count >= 11714 && cycle_count <= 11722) begin
            $display("DBG cyc=%0d pc=%08x preif=%0b ready=%0b cap=%0b imemPreV=%0b qPreV=%0b if=%08x:%08x ifpre=%08x:%08x id_use=%0b id=%08x:%08x ex=%08x br=%0b pred=%0b act=%0b mis=%0b red=%0b/%08x killq=%0b kfr=%0b block=%0b stall=%0b hold=%0b busre=%0b s0ctrl=%0b", cycle_count, u_cpu.preif_pc_addr, u_cpu.preif_valid, !u_cpu.imem_bus_re, u_cpu.if_pre_capture_en, u_cpu.if_pre_valid_imem, u_cpu.if_pre_valid, u_cpu.if_instr_addr, u_cpu.if_instr, u_cpu.if_pre_instr_addr, u_cpu.if_pre_instr, u_cpu.id_use_if_now, u_cpu.id_instr_addr, u_cpu.id_instr, u_cpu.ex_instr_addr, u_cpu.ex_branch_flag, u_cpu.ex_pred_taken, u_cpu.ex_actual_jump_flag, u_cpu.ex_mispredict, u_cpu.frontend_redirect_flag, u_cpu.frontend_redirect_addr, u_cpu.frontend_redirect_kill_q, u_cpu.u_if_id.kill_fetch_r, u_cpu.pipeline_block, u_cpu.pipeline_stall, u_cpu.pipeline_hold, u_cpu.imem_bus_re, u_cpu.slot0_ctrl_redirect);
        end
        if (cycle_count == 11730) $finish;
    end
