...
module cpu_tb;
    ...
    always @(posedge clk) begin
        if (cycle_count >= 11712 && cycle_count <= 11728) begin
            $display("DBG cyc=%0d pc=%08x preif=%0b if=%08x:%08x ifpreV=%0b ifpre=%08x:%08x id_use=%0b id=%08x:%08x idpred=%0b/%08x ex=%08x br=%0b j=%0b pred=%0b/%08x act=%0b/%08x mis=%0b red=%0b/%08x killq=%0b kfr=%0b s0br=%0b s0ctrl=%0b s1=%0b flush=%0b wb=%0b/%0d/%08x memwe=%0b/%08x/%08x", cycle_count, u_cpu.preif_pc_addr, u_cpu.preif_valid, u_cpu.if_instr_addr, u_cpu.if_instr, u_cpu.if_pre_valid, u_cpu.if_pre_instr_addr, u_cpu.if_pre_instr, u_cpu.id_use_if_now, u_cpu.id_instr_addr, u_cpu.id_instr, u_cpu.id_pred_taken, u_cpu.id_pred_target, u_cpu.ex_instr_addr, u_cpu.ex_branch_flag, u_cpu.ex_jump_flag, u_cpu.ex_pred_taken, u_cpu.ex_pred_target, u_cpu.ex_actual_jump_flag, u_cpu.ex_actual_jump_addr, u_cpu.ex_mispredict, u_cpu.frontend_redirect_flag, u_cpu.frontend_redirect_addr, u_cpu.frontend_redirect_kill_q, u_cpu.u_if_id.kill_fetch_r, u_cpu.slot0_branch_redirect, u_cpu.slot0_ctrl_redirect, u_cpu.slot1_pred_redirect, u_cpu.pipeline_flush, u_cpu.wb_regs_we, u_cpu.wb_regs_w_addr, u_cpu.wb_actual_regs_w_data, u_cpu.mem_dmem_we, u_cpu.mem_dmem_wr_addr, u_cpu.mem_dmem_w_data);
        end
        if (cycle_count == 11740) $finish;
    end
