...
module cpu_tb;
    ...
    always @(posedge clk) begin
        if (cycle_count >= 11718 && cycle_count <= 11722) begin
            $display("DBG cyc=%0d expc=%08x id=%08x:%08x if=%08x:%08x ifpreV=%0b ifpre=%08x:%08x wb=%0b/%0d/%08x exregswe=%0b exw=%0d/%08x memwe=%0b pred=%0b act=%0b", cycle_count, u_cpu.ex_instr_addr, u_cpu.id_instr_addr, u_cpu.id_instr, u_cpu.if_instr_addr, u_cpu.if_instr, u_cpu.if_pre_valid, u_cpu.if_pre_instr_addr, u_cpu.if_pre_instr, u_cpu.wb_regs_we, u_cpu.wb_regs_w_addr, u_cpu.wb_actual_regs_w_data, u_cpu.ex_regs_we, u_cpu.ex_regs_w_addr, u_cpu.ex_regs_w_data, u_cpu.mem_dmem_we, u_cpu.ex_pred_taken, u_cpu.ex_actual_jump_flag);
        end
        if (cycle_count == 11725) $finish;
    end
